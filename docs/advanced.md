...
void save_and_print(const char* s, size_t len, void* data) {
    CallbackData* cd = (CallbackData*)data;
    fwrite(s, 1, len, cd->file);
    printf("%.*s", (int)len, s);
    fflush(stdout);
    cd->chunk_count++;
}
...
c->request_stream(c, save_and_print, &data);
...
Session* create_session(const char* api_key) {
    Session* s = malloc(sizeof(Session));
    s->client = llmapi_client_create(api_key, LLMAPI_URL_POE);
    snprintf(s->session_id, sizeof(s->session_id), "%ld", time(NULL));
...
1. **Reuse Client Objects** - Creating clients is expensive
   ```cpp
   // Good: One client for many requests
   Client client(key, url);
   ...
   for (auto& query : queries) {
       Client client(key, url);  // Wasteful!
       client.user(query).request();
   }
   ```

2. **Stream for Large Responses** - Better UX and memory usage
   ```cpp
   // For long responses, streaming is better
   client.request([](auto chunk) { print(chunk); });
   
   // Non-streaming loads everything in memory first
   auto response = client.request();
   ```

3. **Clear History When Needed** - Reduces token usage
   ```cpp
   // For unrelated queries, clear history
   client.clear().user("New topic").request();
   ```

4. **Batch Independent Requests** - Use multiple clients
   ```cpp
   std::vector<std::future<std::string>> futures;
   for (auto& query : queries) {
       futures.push_back(std::async([&, query] {
           Client client(key, url);
           client.user(query).request();
           return client.getAnswer();
       }));
